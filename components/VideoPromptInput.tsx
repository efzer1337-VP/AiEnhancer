          ...
          <div className="flex items-center justify-between">
            ...
            <button
              ...
              className={`relative inline-flex h-5.5 w-10.5 items-center rounded-full transition-all duration-300 focus:outline-none cursor-pointer ${
                webSearch ? 'bg-purple-600 shadow-[0_0_12px_rgba(168,85,247,0.4)]' : 'bg-white/10 hover:bg-white/15'
              }`}
              disabled={isLoading}
            >
              <span
            ...
            <div className="grid grid-cols-4 gap-1">
              {(['off', 'low', 'medium', 'high'] as const).map((lvl) => {
                ...
                const activeStyles = {
                  ...
                  low: 'bg-purple-500/15 border-purple-500/40 text-purple-300',
                  medium: 'bg-pink-500/15 border-pink-500/40 text-pink-300',
                  high: 'bg-indigo-500/15 border-indigo-500/40 text-indigo-300',
                };
